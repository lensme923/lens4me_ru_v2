...
class SaleCrmSiteMaster extends \CBitrixComponent
{
	...
	protected function initSteps()
	{
		...
		if ($notExistModules)
		{
			...
			{
				// Activate coupon
				$this->addWizardStep("Bitrix\Wizard\Steps\ActivationKeyStep", 340);
			}
			else
			{
				// Update system
				$this->addWizardStep("Bitrix\Wizard\Steps\UpdateSystemStep", 350);
		...
		else
		{
			...
			{
				$this->addWizardStep("Bitrix\Wizard\Steps\ModuleStep", 360);
			...
			else
			{
				...
				{
					$this->addWizardStep("Bitrix\Wizard\Steps\ModuleStep", 360);
					$this->addWizardStep("Bitrix\Wizard\Steps\ModuleInstallStep", 370);
	...
	protected function checkPersonType()
	{
		...
		{
			$this->addWizardStep("Bitrix\Wizard\Steps\PersonTypeStep", 330);
		}
	}
	...
	protected function checkAgents()
	{
		...
		if (!$result->isSuccess())
		{
			$this->addWizardStep("Bitrix\Wizard\Steps\AgentStep", 310);
	...
	private function checkB24Connection()
	{
		...
		if ($b24connector->isModule())
		{
			...
			{
				$this->addWizardStep("Bitrix\Wizard\Steps\B24ConnectorStep", 320);
			}
			else
			{
	...
	private function checkPushServer()
	{
		...
		if ($pushChecker->isModuleLoaded())
		{
			...
			{
				if ($pushChecker->isPushActive())
				{
					if (!$pushChecker->isShared())
					{
						// after ModuleStep
						$this->addWizardStep("Bitrix\Wizard\Steps\PushAndPullStep", 365);
					}
				}
				else
				{
					...
					if (!$registerResult->isSuccess())
					{
						...
						{
							$this->addError($errorMessage, self::ERROR_TYPE_WIZARD);
						}
					}
				}
			}
