		...
		if($this->CheckParams($tmpdir))
		{
			...
			while(!empty($arFolders) && !$fid)
			{
				...
				if(!empty($mailsIds))
				{
					...
					{
						$mailId = $mailsIds[$i];
						if(($arMailFile = $this->imap->getMessageFile($folder, $mailId, $this->params, array('txt', 'csv', 'xls', 'xlsx', 'xlsm', 'zip', 'gz', 'tgz', 'rar'), ($error='')))!==false)
						{
							...
							if($this->params['DELETE_OLD_MAIL']=='Y')
							{
