...
class Conversion
{
	...
	public function GetElementFieldsEx($arFilter, $loadElemFields)
	{
		...
		$dbResElements = \Bitrix\KdaImportexcel\DataManager\IblockElement::GetList($arFilter, $arSelectElementFields, array(), 1);
	...
	public function GetPropertyElementValue($arProp, $val, $relField = '')
	{
		if($val)
		{
			...
			if(!isset($this->propVals[$arProp['ID']][$selectField][$val]))
			{
				...
				$dbRes = \Bitrix\KdaImportexcel\DataManager\IblockElement::GetList(array("ID"=>$val), array($selectField));
