...
class Product
{
	...
	public function SaveStore($ID, $IBLOCK_ID, $arStores)
	{
		...
		{
			if(isset($arFieldsStore['AMOUNT']) && $arFieldsStore['AMOUNT']!='-') $arFieldsStore['AMOUNT'] = $this->GetFloatVal($arFieldsStore['AMOUNT']);
	...
	public function SetProductQuantity($ID, $IBLOCK_ID=0)
	{
		...
		if($asSumStore)
		{
			if($arRes = \Bitrix\Catalog\StoreProductTable::getList(array('filter'=>array('STORE.ACTIVE'=>'Y', 'PRODUCT_ID'=>$ID),'group'=>array('PRODUCT_ID'), 'runtime'=>array(new \Bitrix\Main\Entity\ExpressionField('SUM', 'SUM(AMOUNT)')), 'select'=>array('SUM')))->Fetch())
			{
	...
	public function GetProductQuantity($ID, $IBLOCK_ID)
	{
		...
		if($arProduct = $this->GetList(array(), array('ID'=>$ID), false, false, array('ID', 'QUANTITY', 'TYPE'))->Fetch())
		{
			if(defined('\Bitrix\Catalog\ProductTable::TYPE_SKU') && $arProduct['TYPE']==\Bitrix\Catalog\ProductTable::TYPE_SKU && $this->saveProductWithOffers===false)
			{
				...
				{
					$quantity = (int)$arOffer['CATALOG_QUANTITY'];
				}
			}
			else
			{
