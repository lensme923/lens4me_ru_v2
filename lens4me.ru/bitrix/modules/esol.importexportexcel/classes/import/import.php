...
class CKDAImportExcel {
	...
	public function MissingElementsUpdate($ID, $IBLOCK_ID, $isOffer = false)
	{
		...
		$this->AfterSaveProduct($arElementFields, $ID, $IBLOCK_ID, true);
	...
	public function SaveRecord($arItem)
	{
		...
		if($this->params['ELEMENT_NO_PRICE_DEACTIVATE']=='Y' && $this->IsEmptyPrice($arFieldsPrices))
		{
		...
		$dbRes = \Bitrix\KdaImportexcel\DataManager\IblockElement::GetList($arFilter, $arKeys);
		...
		$allowCreate = (bool)(\Bitrix\KdaImportexcel\DataManager\IblockElement::SelectedRowsCount($dbRes)==0 && $this->params['ONLY_DELETE_MODE']!='Y');
	...
	public function AddElement($arFieldsElement)
	{
		$el = new CIblockElement();
	...
	public function SaveSKUWithGenerate($ID, $NAME, $IBLOCK_ID, $arItem)
	{
		...
		if(!empty($this->fieldsForSkuGen))
		{
			...
			foreach($filedList as $key=>$field)
			{
				...
				if((preg_match('/^OFFER_(IE_PREVIEW_PICTURE|IE_DETAIL_PICTURE|ICAT_QUANTITY|ICAT_PURCHASING_PRICE|ICAT_PRICE\d+_PRICE|ICAT_STORE\d+_AMOUNT|ICAT_WEIGHT)$/', $field) || in_array($field, $arGenFields)) && strpos(preg_replace('/\{[^\}]*\}/', '', $val), $this->params['ELEMENT_MULTIPLE_SEPARATOR'])!==false)
				{
					$arItem['~~'.$key] = $valOrig;
					$arItem[$key] = $val;	
	...
	public function SaveSKU($ID, $NAME, $IBLOCK_ID, $arItem, $convertedFields=array())
	{
		...
		if($dbRes->SelectedRowsCount()==0 && $ID && $this->params['SEARCH_OFFERS_WO_PRODUCTS']!='Y')
		{
			if($this->params['ONLY_UPDATE_MODE_ELEMENT']!='Y')
			{
				...
				if($this->CheckRequiredProps($arFieldsProps, $OFFERS_IBLOCK_ID) && ($OFFER_ID = $this->AddElement(array_merge($arFieldsElement, array('PROPERTY_VALUES'=>array($OFFERS_PROPERTY_ID => $ID))))))
				{
		...
		foreach($currentUid as $tuid)
		{
			...
			elseif(strpos($tuid, 'IP_PROP')===0)
			{
				...
				else
				{
					...
					elseif($propsDef[$uid]['PROPERTY_TYPE']=='E')
					{
						$valUid = $this->GetIblockElementValue($propsDef[$uid], $valUid, $fs);
						$valUid2 = $this->GetIblockElementValue($propsDef[$uid], $valUid2, $fs);
						$canSubstring = false;
					}
					$uid = 'PROPERTY_'.$uid;
				}
	...
	public function GetStoreAmountList(&$arFieldsProductStores, $arParams, $value)
	{
		...
		foreach($arStores as $strStore)
		{
			...
			{
				$dbRes = \Bitrix\Catalog\StoreTable::getList(array('filter'=>array('LOGIC'=>'OR', array('TITLE'=>$storeName), array('ADDRESS'=>$storeName)), 'select'=>array('ID')));
	...
	public function GetFileArray($file, $arDef=array(), $arParams=array())
	{
		...
		$checkFormat = false;
		if($bNeedImage) $fileTypes = array('jpg', 'jpeg', 'png', 'gif', 'bmp', 'svg', 'webp');
	...
	public function SaveProperties($ID, $IBLOCK_ID, $arProps, $needUpdate = false, $arFieldsElement=array())
	{
		...
		if(!empty($arProps) && $this->params['ELEMENT_IMAGES_FORCE_UPDATE']!='Y')
		{
			...
			foreach($arOldProps as $pk=>$pv)
			{
				...
				elseif(in_array($propsDef[$pk]['PROPERTY_TYPE'], array('L', 'E', 'G')) && $propsDef[$pk]['MULTIPLE']=='Y' && is_array($arProps[$pk]) && is_array($pv) && !isset($pv['VALUE']) && count(array_diff($arProps[$pk], $pv))==0 && count(array_diff($pv, $arProps[$pk]))==0)
				{
					unset($arProps[$pk]);
				}
				else
	...
	public function GetIblockElementValue($arProp, $val, $fsettings, $bAdd = false, $allowNF = false)
	{
		...
		if($relField && $relField!='IE_ID' && $arProp['LINK_IBLOCK_ID'])
		{
			...
			$dbRes = \Bitrix\KdaImportexcel\DataManager\IblockElement::GetList($arFilter, array('ID'), array('ID'=>'ASC'), 1);
	...
	public function AfterSaveProduct(&$arFieldsElement, $ID, $IBLOCK_ID, $isUpdate=false)
	{
		if($this->params['ELEMENT_NO_QUANTITY_DEACTIVATE']=='Y' && intval($this->productor->GetProductQuantity($ID, $IBLOCK_ID))==0)
		{
...
class KDAChunkReadFilter implements KDAPHPExcel_Reader_IReadFilter
{
	...
	{
		if(preg_match('/^([A-Z]+)(\d+):([A-Z]+)(\d+)$/', trim($mergeRef), $m))
		{
